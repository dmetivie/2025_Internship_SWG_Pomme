...
""" 
MiddleMonth(year::Int)=(cumsum([0;DaysPerMonth(year)])[2:13] .+ cumsum([0;DaysPerMonth(year)])[1:12]) ./ 2

"""
    PlotYearCurves(curvesvec::Vector{Vector{Float64}}, labelvec::Vector, title::String="", bands::Vector=[], colorbands::Vector=[])
...
"""
function PlotYearCurves(curvesvec::Vector, labelvec::Vector, title::String="", bands::Vector=[], colorbands::Vector=[])
    fig=Figure()
...
PlotYearCurves(curvesvec::Vector{Float64},labelvec::String,title::String) = PlotYearCurves([curvesvec],[labelvec],title)
PlotYearCurves(curvesvec::Vector{Float64},labelvec::Vector,title::String="",bands::Vector=[],colorbands::Vector=[]) = PlotYearCurves([curvesvec],labelvec,title,bands,colorbands)

"""
    PlotParameters(Parameters_vec::Vector)
...
"""
function PlotParameters(Parameters_vec::Vector)
    ...
    for (j,Parameters) in enumerate(Parameters_vec)
        ...
        ax.title="Real $(str) vs estimated $(str) with 3 methods"
